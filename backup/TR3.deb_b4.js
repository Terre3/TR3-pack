				...
				<div id="3doptions" style="border: solid;margin: 5px; padding: 5px; display: none;float:left">\
					...
					<div id="sroOpt" style="display:none">\
						...
						<div id="physicsDiv" style="display:none;border: 1px dashed gray;padding: 1px;float: left;width: 100%;height:150px;overflow:auto;">\
							...
							<span>·&nbsp;Impulso: </span><input id="impulsePtlZone" type="text" style="width: 65px;" value="30">\
						...
						<div id="textIntoDiv" style="display:none;border: 1px dashed gray;padding: 1px;float: left;width: 100%;">\
							...
							<input id="buttonTextInto" type="button" value=" Insertar Texto " style="" class="">\
							<input id="del_vGeom2" type="button" value="Borrar último" class="">\
						</div>\
						<div style="float:left">\
							...
							<span>&nbsp;- Exportar ⇩ </span><select id="exporter-type" onchange="TR3.changeExportType()">\
								...
								<option value="obj3d">Sin Edificios</option>\
...
TR3.setEvtPanel = function () {
	...
	document.getElementById('del_vGeom2').addEventListener("click", function () { TR3.del_vGeom(false, true); document.getElementById('metrics').checked = false; }, false);
...
TR3.animate = function () {
	...
	for (var i = 0; i < TR3.locurote.length; i++) {
		var locuI = TR3.locurote[i];
		var position = locuI[0].position;
		var unfix = locuI[4] || 0;
		var z = locuI[3] || position.y;
		z += Math.sin(elapsedTime * locuI[1] + unfix) * locuI[2];
		position.x = z;
		position.y = z;
		position.z = z;
	}
...
TR3.exportGLTF = function (type) {
	...
	if (type == 'scene') {
		...
		for (var i = 0; i < TR3.scene.children.length; i++) {
			var obj = TR3.scene.children[i]
			if (obj.type === 'Mesh' || obj.type === 'Group' || obj.type === 'Object3D')
				if(	obj.name !== "Builds" && obj.name !== 'TransformControls' && obj.name !== 'G-hills' && obj.name !== 'cursor' && obj.name !== "planetary" && obj.name !== "starField"  && obj.name !== "Sky" && obj.name !== "Compass" && obj.name !== "N" && obj.name !== "S" && obj.name !== "E" && obj.name !== "W")
						exporter.push(exportObjItem(obj));
		};
	...
	for (var i = 0; i < objSlct.length; i++) {
		...
		if (intsct.length > 0 && TR3.slctBox.length === 0) {
			...
			if (TR3.optionsSet.tentative && oSlctiChild.TR3 && oSlctiChild.TR3.hitbox) {
				...
				if (oSlctiChild.TR3.floorMesh) { color = 0x777777; }
				if (oSlctiChild.TR3.transformMesh) { color = 0x0000ff; }
				if (oSlctiChild.TR3.mass) { color = 0x00ff00; }
...
TR3.click_Obj3d = function (evt) {
	...
	if (/*&& TR3.transformControls.active == false && */TR3.optionsSet.tentative) {
...
TR3.del_vGeom = function (all, last) {
	...
	if (TR3.vGeom.sprites && TR3.vGeom.sprites.length) {
		...
		if (i === 0) TR3.vGeom.sprites = false;
	}
	...
	if (TR3.transformControls.enabled) {
		switch (evt.keyCode) {
			...
			case 79: // O
				...
				obj3d.position.y = pos[4] + (obj3d.TR3.box.max.y - obj3d.TR3.box.min.y)/2*obj3d.scale.y;
				break;
			case 86: // V
				var obj3d = TR3.transformControls.object;
			...
			case 76: // L
				...
				var text = '<div id="infohelp3dObj" style="padding:10px 40px">\
								<div align="center"><u>Valores Geométricos:</u></div>\
...
TR3.UpdateGeom3D = function (type) {
	...
	if (type == 1) {
		...
		obj3d.scale.x = obj3d.scale.x * Width3D / width;
		obj3d.scale.y = obj3d.scale.y * height / Height3D;
		obj3d.scale.z = obj3d.scale.z * Depth3D / depth;
	}
	else if (type == 3) {
		obj3d.rotation.x = 0;
		obj3d.rotation.y = 0;
...
TR3.keyScape = function () {
	...
	if (typeof obj3d.clippingPlanes == 'boolean') {
		TR3.controls.enableZoom = true;
	...
	if (typeof obj3d.transparentTC == 'boolean') {
	...
	if (typeof obj3d.TR3.transformMesh === 'number') {
	...
	if (typeof obj3d.edgesTC == 'boolean') {
		obj3d.edgesTC = false;
...
TR3.getCoordsDistance3D = function (p1, p2) {
	...
	var dAlt = (p2[2] - p1[2]) * 1 / TR3.adjustScale;;
...
TR3.setCloneObj = function (obj3d) {
	...
	clon.position.set(coord.x, - 1 * obj3d.scale.y + coord.y, coord.z);
	...
	/*if (clonGltf.animations.length > 0 && animation) { TransformControls.object...
...
TR3.makeClon = function (gltf, obj3d, posObj3d, scale, animation, extraData, slctItem, aRotate) {
	...
	if (obj3d.TR3.mass || typeof (obj3d.TR3.mass) == "number") {
		clon.TR3.mass = obj3d.TR3.mass;
		TR3.pscs.bricks.push(clon);
	}
	...
	if (clonGltf.animations.length > 0 && animation) {
		clonGltf.mixer = new THREE.AnimationMixer(clon);
		var action = clonGltf.mixer.clipAction(clonGltf.animations[clon.TR3.animateAction]);
		action.play();
	}
...
TR3.loaderGLTFsucces = function (gltf, dfd_LoadFile, params) {
	...
	if (ext === 'ifc') {
		...
		objGLTF.children[0].position.set(offsetPos.x,offsetPos.y,offsetPos.z);
	}*/
	...
	if (gltf.animations && gltf.animations.length > 0 && animation) {
		...
		var action = gltf.mixer.clipAction(gltf.animations[animateAction]);
	...
	if (posFalse === true) coord.y = TR3.operaValues(( -1 * objGLTF.TR3.box.min.y)/2 * objGLTF.scale.y, coord.y, 'suma');
	objGLTF.position.set(coord.x, coord.y, coord.z);
	//var shadow = TR3.setShadowBase(objGLTF);

	if (mass || typeof (mass) == "number") {
		if (TR3.pscs.physicsWorld)
...
TR3.transCtrlsEnabled = function (boo, type) {
	...
	if (boo) {
		if (TR3.transformControls.object && TR3.transformControls.object.TR3)
			TR3.transformControls.object.TR3.TCactive = true;
		TR3.onTransformEvTrue(TR3.transformControls);
		TR3.lock3d('obj3d');
	} else {
		TR3.unlock3d('obj3d');
		var object = TR3.transformControls.object;
		if (object) {
