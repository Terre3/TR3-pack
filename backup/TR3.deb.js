				...
				<div id="3doptions" style="border: solid;margin: 5px; padding: 5px; display: none;float:left">\
					...
					<div id="orbitalMoves" style="width: 155px; text-align: center; box-shadow: rgb(136, 136, 136) 1px 1px 5px; border: medium outset #eee; float: left;">\
						...
						<div style="text-align: left; margin: 5px">\
							...
							<input id="malla" type="checkbox" class="changeOpt"> Malla\
							<input id="tentative" type="checkbox" class="changeOpt" checked> Tentativo\
							<input id="cheapMode" type="checkbox" class="changeOpt"> Mínimo\
						</div>\
					</div>';
				var pnl2 = "";
				var pnl3 = "";
...
TR3.setStart = function( params ){
	...
	TR3.setMeshMap( ori, bbox, options, {magnification: values, lookAt: TR3.config.lookAt} );
...
TR3.getOptions = function(){		
	var imgControlOpt, cursor3dOpt, anaglyphOpt, autoRotateOpt, wireframeMeshOpt, tentativeOpt, cheapModeOpt;
	...
	if(document.getElementById('cheapMode')){
		cheapModeOpt = document.getElementById('cheapMode').checked;
	}else{
		cheapModeOpt = false;
	}
	
	return {imgControl: imgControlOpt, cursor3d: cursor3dOpt, anaglyph: anaglyphOpt, autoRotate: autoRotateOpt, wireframeMesh: wireframeMeshOpt, tentative:tentativeOpt, cheapMode: cheapModeOpt}
};
...
TR3.setOpts = function(change){
	...
	if(change){
		...
		if(change.cheapMode != undefined){options.cheapMode = change.cheapMode;}
	}
	TR3.setOptions(options.imgControl, options.cursor3d, options.anaglyph, options.autoRotate, options.wireframeMesh, options.tentative, options.cheapMode);
};

TR3.setOptions = function(imgControlOpt, cursorOpt, anaglyphOpt, autoRotateOpt, wireframeMeshOpt, tentativeOpt, cheapModeOpt){
	...
	if( cheapModeOpt == true ){
		document.getElementById('cheapMode').checked = true;
	}else{
		document.getElementById('cheapMode').checked = false;
	}
	...
	if(TR3.map){
		...
		if( cheapModeOpt == true ){
			//TR3.optionsSet.tentative = true;
		}else{
			//TR3.optionsSet.tentative = false;
		}
		//TR3.renderScene();//anaglyph
	}
	/*Anaglyph*/
	TR3.optionsSet = {imgControl: imgControlOpt, cursor3d: cursorOpt, anaglyph: anaglyphOpt, autoRotate: autoRotateOpt, wireframeMesh: wireframeMeshOpt, tentative: tentativeOpt, cheapMode: cheapModeOpt};
	...
	if (objSlcted.name == 'mesh3d') {
		...
		for (var i = 0; i < objSlcted.length; i++) {
			...
			if (intersectsTry.length > 0 && typeof (oriScale) != "string") {
				...
				if (TR3.optionsSet.tentative == true) {
...
TR3.keyDown = function (evt) {
	...
	if (TR3.transformControls.enabled) {
		switch (evt.keyCode) {
			...
			case 70: // F
				...
				obj3d.wireframe = !obj3d.wireframe;
				if (obj3d.wireframe) {
					obj3d.traverse(function (child) {
						...
						if (child.isMesh) {
							...
							mesh.name = 'meshWireframe';
							TR3.scene.add(mesh);*/

							// Setup our wireframe
							var wireframeGeometry = new THREE.WireframeGeometry(geometry);
							var wireframeMaterial = new THREE.LineBasicMaterial({ color: 0xcccccc });
					...
					//TR3.scene.remove(TR3.scene.getObjectByName('wireframe')); Not working...
					obj3d.traverse(function (child) {
	...
	if (TR3.camera.visible == true && evt.keyCode == '17') {
		...
		TR3.plasmaBalls.push(plasmaBall);
	...
	if (evt.keyCode == '16') {
		...
		if (TR3.camera.visible == false) {
			for (var i = 0; i < TR3.plasmaBalls.length; i++) {
				TR3.scene.remove(TR3.plasmaBalls[i]);
			}
			TR3.plasmaBalls = [];
		}
	}
...
TR3.optionsSet = { imgControl: true, cursor3d: false, anaglyph: false, autoRotate: false, wireframeMesh: false, tentative: true, cheapMode: false };
...
TR3.animate = function () {
	...
	var cheapMode = TR3.optionsSet.cheapMode;
	...
	if ( cheapMode==false && TR3.vGeom && TR3.vGeom.obj3d) {
		var flag = false;
		for (var i = 0; i < TR3.vGeom.obj3d.length; i++) {
	...
	if ( cheapMode==false && TR3.plasmaBalls.length > 0 && TR3.camera.visible == true ) {
		for(var i=0; i<TR3.plasmaBalls.length; i++){
			TR3.plasmaBalls[i].translateZ(-100 * delta);
		}
	}
	...
	var zone = TR3.zone;
	if ( cheapMode==false && TR3.moon/* && TR3.sun*/ ) {
...
TR3.setMeshMap = function (imgOri, bboxImgOri, optionsSet, valuesSet) {
	...
	if (TR3.bboxImgOri && JSON.stringify(bboxImgOri) == JSON.stringify(TR3.bboxImgOri) && TR3.loadingDiv && TR3.canvasDest) {
		...
		var imgControl, cursor3d, anaglyph, magnification, autoRotate, wireframeMesh, tentative, cheapMode;
		...
		cheapMode = optionsSet.cheapMode
		...
		TR3.persist2Scene();
		TR3.setOptions(imgControl, cursor3d, anaglyph, autoRotate, wireframeMesh, tentative, cheapMode);
	}
	...
	if (TR3.scene && TR3.scene.children)
		for (var i = 0; i < TR3.scene.children.length; i++) {
			if (typeof (TR3.scene.children[i].position.y) == 'string') {
				var z = TR3.scene.children[i].position.y.replaceAll('$', '');
				var zLast = z.slice(-1);
				var znum = z.slice(0,-1);
				var zPos;
				if (z && zLast == 'T') {
					zPos = TR3.zM2T(znum*1, true);
				} else {
					zPos = z.replace(zLast, '');
				}
				TR3.scene.children[i].position.y = zPos * 1;
			}
		}
...
TR3.unlock3d = function (cause) {
	...
	TR3.makeObjects = function(imgConteint){
		...
		var starsMaterial = new THREE.PointsMaterial( { color: '#cccc88', size: 10000 } );
		TR3.starField = new THREE.Points( starsGeometry, starsMaterial );
		TR3.starField.name = 'starField';
